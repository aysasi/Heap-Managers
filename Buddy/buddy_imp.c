...
#define MAX_ORDER 25		  // maximum order ;
#define MAX_SIZE (1 << MAX_ORDER) // total and maximum size = 2^K_MAX
#define MIN_ORDER 4 //for blocks of order smaller or equal to 4 addresses where not stored correctly.
...
void *malloc(size_t size)
{
	...
	if (block)
	{ // not null, so a block has been found or created
		block->free = 0;
		return (block + 1);
	}
...
void *realloc(void *ptr, size_t size)
{
	...
	if ((1 << block->order) >= s + METADATA) {  //if new size fits in the existing block, no need to create a new one
		return ptr;
	}
	...
	if (new_location)
	{
		...
		//memcpy(new_location, ptr, (1 << block->order) - META_SIZE); // copies data from old region to new
		dllist_t *new_block = (((dllist_t *)new_location) - 1);
		memmove(new_location, ptr, (1 << new_block->order));
...
dllist_t *half(dllist_t *block, int new_order)
{
	...
	while (block->order > new_order)
	{
		...
		add_node(next_block);
		add_node(block);
		//DEspues de esta llamadaa el order de next block se cambia por algun motivo
		//printf("·Hola");
	}
	return block;
}
...
void merge(dllist_t *block)
{
	...
	int x = 0;
	if (left_bud != NULL)
	{ // it is going to merge block with its left buddy
		x = left_bud->free;
		if (x == 1) {
			if (left_bud->order == block->order) {
...
void add_node(dllist_t *node)
{
	...
	if (!free_nodes[o])
	{ // there isnt any free node of this size yet
		free_nodes[o] = node;
		node->next = NULL;
		node->prev = NULL;
	}
	else
	{ // we add the node to the existing nodes of this size
        dllist_t *node_aux = node->prev;
        while (node_aux != NULL) {
            node = node_aux;
